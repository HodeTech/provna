...
- **ASSEMBLE the machinery.** Hash chains, Merkle trees, transparency logs (Rekor/Trillian), RFC3161 timestamping, and JSON canonicalization (RFC8785) are commodity, well-understood, independently audited primitives. Reinventing any of them would be cost without defensibility. Provna's contribution at the mechanism layer is **integration discipline and fail-closed assembly**, not cryptographic novelty.
...
flowchart TD
    ...
    F --> G["External anchor of root"]
    G --> G1["Rekor / Trillian transparency log"]
    G --> G2["RFC3161 TSA timestamp token"]
...
- **Merkle root + external anchor.** Chain segments are batched into a Merkle tree; the root is published to a transparency log (Rekor/Trillian) and timestamped by an RFC3161 TSA. This provides an **independent time source** and **third-party inclusion proof** that does not depend on Provna's or the customer's clock.
...
sequenceDiagram
    ...
    participant Anchor as External anchor Rekor TSA
    ...
    Anchor-->>S4: inclusion proof and timestamp token
    S4-->>P: evidence record with portable witness and policy snapshot ref
```

Note the ordering: S4 records the outcome **and** waits on the external anchor for the inclusion proof. Anchoring is part of producing the evidence, not a best-effort background task that can be silently skipped.

---

## 5. Why the External Anchor Is Critical

The single most important design decision in S4 is the **external anchor**, because it closes the threat every competitor leaves open: **the insider / key-holder who rewrites history.**
...
flowchart LR
    ...
    subgraph Anchored["Externally anchored ledger -- Provna"]
        P1["Merkle root published to transparency log + RFC3161 TSA"] --> P2["Independent time + third party inclusion proof"]
        P2 --> P3["Any rewrite contradicts the prior anchor"]
        P3 --> P4["Rewrite DETECTED"]
    end
```

Provna closes the gap by anchoring the Merkle root to an **independent transparency log (Rekor/Trillian)** and an **independent RFC3161 timestamp authority**. Once a root is anchored, the historical state at that point in time is witnessed by a third party. Any later rewrite produces a chain whose roots no longer match the previously anchored values - the contradiction is externally observable. The key-holder can stop new records, but **cannot silently alter the witnessed past.**
...
|---|---|---|
| **EU AI Act Article 12** (record-keeping / forensic reproducibility) | Automatic, traceable records sufficient to reconstruct events | Per-action canonical (JCS) decision event + `policy_snapshot_ref` (reproduce the *why*) + hash-chain + Merkle + external anchor (reproduce the *what* and prove it is unaltered) |
...
| **DORA** (operational resilience, long retention) | Durable, integrity-protected operational records | Anchored chain + optional PQC-hybrid signature for long-retention integrity (Section 9) |
...
- If canonicalization, signing, chaining, or the mandatory anchor step cannot complete, the action's evidence path is treated as a failure - there is **no downgrade to unsigned/unanchored logging.**
