...
**The rule.** Audit output is not a log. Every governed action produces a tamper-evident record: hash-chained events, a Merkle root, an **external anchor** (Rekor / Trillian plus an RFC3161 timestamp), RFC8785 JCS canonicalization, a `kid`-embedded portable witness, and a `policy_snapshot_ref` pinning the exact policy in force. A governance-failure signal is itself persisted as a **signed audit event**. This signed, anchored evidence — not any internal database, not any plaintext log — is the authoritative system of record, and it maps to EU AI Act Article 12 (forensic reproducibility) and Article 14 (human oversight), plus DORA and MiFID obligations.
