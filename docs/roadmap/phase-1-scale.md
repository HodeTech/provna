...
- **Production-target stack live for at least one production deployment.** Inline PEP in Go (Rust reserved for a future hot leaf with a proven trigger) on the hot path; DBOS as the durable substrate (Temporal kept as a seam-isolated contingency triggered only by multi-tenant fan-out / a Postgres ceiling / a buyer mandate, NOT a scheduled migration); Cedar (OpenFGA deferred behind a relationship-resolver interface until a partner is provably ReBAC) + biscuit/macaroon attenuation as the PDP/delegation layer; a self-hosted transparency log (Tessera) + an internal HSM-backed RFC3161 TSA + a cross-organization witness cosignature (Rekor v2 as the reference design) for evidence anchoring; K8s/Helm deployment into a customer VPC. The MVP single-container stack is retired for production traffic (see [../tech-stack.md](../tech-stack.md)).
...
**What:** Migrate from the MVP single-container stack (TS/Python + DBOS + Postgres + Cedar) to the production-target stack (see [../tech-stack.md](../tech-stack.md)):
- **Inline PEP in Go** (Rust reserved for a future hot leaf with a proven trigger) on the hot path, replacing the MVP inline path, to meet money-path latency SLOs without a fail-open downgrade.
- **DBOS** as the durable execution substrate at scale (consumed, not rebuilt - compensation is layered on top; Temporal kept as a seam-isolated contingency triggered only by multi-tenant fan-out / a Postgres ceiling / a buyer mandate, NOT a scheduled migration; see [../decisions/0005-s2-dbos-substrate-compensation-library.md](../decisions/0005-s2-dbos-substrate-compensation-library.md)).
- **Cedar + biscuit/macaroon** (OpenFGA deferred behind a relationship-resolver interface until a partner is provably ReBAC) for the PDP and real caveat-attenuation + recursive transitive-revocation with per-hop signature verification (see [../decisions/0006-s3-and-gate-attenuation-behavioral-admission.md](../decisions/0006-s3-and-gate-attenuation-behavioral-admission.md)).
- **A self-hosted transparency log (Tessera) + an internal HSM-backed RFC3161 TSA + a cross-organization witness cosignature** (Rekor v2 as the reference design) for evidence anchoring, with RFC8785 JCS canonicalization and kid-embedded portable witness (see [../decisions/0007-s4-merkle-external-anchor-jcs.md](../decisions/0007-s4-merkle-external-anchor-jcs.md)).
...
**Acceptance:**
- Inline PEP in Go serves at least one production deployment within the money-path latency SLO; the MVP inline path is no longer in the production hot path.
- DBOS-backed sagas run the compensation flow end-to-end (execute -> compensate) in production.
- Cedar + biscuit attenuation and recursive revocation pass adversarial tests (zombie-delegation and missing-signature cases are blocked).
- Evidence is anchored to a self-hosted transparency log (Tessera) with an internal HSM-backed RFC3161 TSA and a cross-organization witness cosignature; an independent verifier can validate a Merkle root and a kid-embedded witness offline.
...
|---|---|---|
| **M1 - Hardened core in production** | Inline PEP (Go) + DBOS + Cedar/biscuit + Tessera/RFC3161-TSA/witness live in one customer VPC; fail-closed verified under fault injection | W2 |
...
1. **Hardened.** The production-target stack (inline PEP Go + DBOS + Cedar/biscuit + Tessera/RFC3161-TSA/witness + K8s/Helm) serves real production traffic, fail-closed, within the money-path latency SLO.
...
| **Hardening regression re-opens fail-closed** | Migrating to inline Go + DBOS + Cedar is exactly where a downgrade path (HMAC self-verify, observe-only shim, revocation fail-open) can silently creep in - the same failure mode that sank competitors. | Fail-closed verified under fault injection on every PEP surface (W2 acceptance); no downgrade path; revocation fail-closed; PreToolUse real-deny preserved. |
...
| **Latency at production scale** | The inline PEP on the money path must meet the SLO without fail-open; a latency miss forces a downgrade that breaks the fail-closed principle and the audit story. | Go hot path (Rust reserved for a future hot leaf with a proven trigger); measure against the money-path SLO as a W2 acceptance criterion; if the SLO cannot be met without downgrade, that is a kill-criterion, not a tuning task. |
