...
| **M3** | S4 signed + anchored evidence-pack prototype: JCS (RFC8785) + Ed25519 + Rekor anchor + kid-embedded + BAR-signal persistence. | [current.md](current.md) / [phase-0-mvp.md](phase-0-mvp.md) |
...
4. **Signed + anchored evidence = system-of-record.** Even Layer-0 audit-only output is signed + Merkle + externally anchored (Rekor/RFC3161) + JCS-canonical + kid-embedded - not plain logging. An insider or key-holder cannot rewrite history.
