...
Provna is planned as a **polyglot monorepo**: a Go/Rust data-plane (the inline hot path), a Python/TS control-plane (orchestration off the critical path), language SDKs, a web panel, the policy + connector-compensation catalog (the moat content), and deploy/docs. The split follows the language strategy in [tech-stack.md](tech-stack.md): inline and fail-closed in Go/Rust; fast-moving orchestration in Python/TS. This is a pre-build proposal, not a frozen layout.

## Package graph

```mermaid
flowchart TB
  subgraph DP["data-plane -- Go / Rust, inline hot path"]
...
```text
provna/
  data-plane/                 # Go/Rust -- inline, hot path, fail-closed
  ...
  control-plane/              # Python/TS -- orchestration, off the critical path
    authz-resolver/           # S3: AND-gate, caveat-attenuation, transitive revocation,
                              #     behavioral/temporal admission (wraps Cedar/OpenFGA PDP)
    compensation-orchestrator/# S2: saga + reverse-saga driver, observe-probe (over DBOS/Temporal)
    audit-assembler/          # S4: hash-chain -> Merkle -> Rekor/Trillian + RFC3161 + JCS witness
  sdk/
...
- **Build vs consume inside the tree.** `ifc-engine` (S1) and `connector-compensation-catalog` (S2) are the real IP. `authz-resolver` wraps a consumed PDP (Cedar/OpenFGA), `compensation-orchestrator` drives a consumed saga substrate (DBOS/Temporal), and `audit-assembler` assembles consumed primitives (OTel/Rekor/Trillian/RFC3161). The canonical boundary is [architecture/build-vs-consume.md](architecture/build-vs-consume.md).
...
2. **action-contract + compensation-orchestrator + one connector** — idempotency + one-click compensation on a single connector (Stripe or NetSuite), over DBOS. This is the S2 moat flywheel's first turn.
3. **authz-resolver + policy** — AND-gate over Cedar/AuthZEN, with dry-run + HITL.
4. **audit-assembler** — hash-chain -> Rekor + the EU AI Act Article 12/14 evidence pack v1.
