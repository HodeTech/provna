...
Considered: **exact-match subset attenuation** (the prior-art approach — a child capability is valid only if its string is a member of the parent's set; rejected: cannot express `financial.*` narrowing to `financial.payment` with an amount cap, so delegation is decorative, not enforced). **Static AND-gate only, no behavioral layer** (rejected: cannot catch the composed-pattern attack class; each request passes, the aggregate is malicious). **Consume a PDP with no resolver around it** (rejected: a raw PDP gives policy evaluation but has no opinion on the user axis, the intent axis, real attenuation, or transitive revocation — exactly the thin slice that is our differentiation). **Standing up OpenFGA as the MVP PDP** (rejected for the MVP: it adds a second network-hop authorization service and a separate failure domain, weakens the air-gap story, and presumes a relationship-based entitlement model no design partner has yet proven they need; deferred behind a resolver interface, see below).

## Decision

S3 = **consume the PDP, build a thin resolver around it**, composed of three built parts plus one orthogonal layer:

1. **Four-axis AND-gate: `agent AND user AND delegation AND intent`.** All four legs must independently authorize the action; any single denial blocks. The *user* leg (on whose behalf the agent acts) and the *intent* leg (the declared business purpose, bound to the action contract) are the axes competitors omit — they are the cheapest part to add and the clearest functional difference. The PDP is **embedded Cedar only for the MVP** — formally verified, evaluated in-process as a single failure domain, lowest latency, and the best air-gap story (no second authorization service to deploy or egress to). Relationships (e.g. `agent member_of team`, `user owns account`) are **modeled as Cedar entities** and resolved through the relationship hierarchy Cedar already supports, surfaced over AuthZEN 1.0. **OpenFGA is deferred behind a relationship-resolver interface** and added only when a design partner's entitlements are *provably* ReBAC (a relationship graph Cedar's entity model cannot express ergonomically); until then the interface is satisfied by the Cedar entity resolver, so the migration is a swap behind a stable seam rather than a rewrite. The resolver consumes Cedar to evaluate the agent and resource policy and fuses the four verdicts. See [../architecture/tech-stack-analysis.md](../architecture/tech-stack-analysis.md) for the full evaluation.
...
- Embedded Cedar (single failure domain, in-process, formally verified) keeps the money-path latency low and the air-gap deployment trivial, while AuthZEN 1.0 alignment keeps engineering focused on the thin differentiating resolver and avoids competing with $740M-class incumbents.
- Deferring OpenFGA behind a relationship-resolver interface avoids a speculative second authorization service while preserving a clean swap-in path if a design partner turns out to be provably ReBAC.

### Negative

- Building the resolver around an embedded PDP adds a fusion-logic surface we must keep correct as AuthZEN evolves, plus a relationship-resolver interface that has to stay swap-clean for a possible future OpenFGA backend.
