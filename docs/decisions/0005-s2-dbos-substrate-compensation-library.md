...
**CONSUME the saga mechanism from DBOS Transact + Postgres NOW for the MVP and early production, strictly behind the gRPC ActionGuard seam plus a thin SagaCoordinator interface. BUILD the moat: a per-connector inverse (A^-1) catalog, an A->A^-1 round-trip test harness, an observe-probe, dry-run, and an API-version-pinned auto-runnable catalog.**

The substrate is held behind two seams so it stays swappable: the gRPC ActionGuard boundary and a thin SagaCoordinator interface. We pre-write a Temporal adapter spike but treat Temporal strictly as a CONTINGENCY - migrate ONLY if a concrete trigger fires (multi-tenant fan-out, a Postgres throughput/latency ceiling, or a buyer mandate). This is explicitly NOT a scheduled DBOS->Temporal migration; DBOS + Postgres is the chosen substrate for the MVP and early production, and the substrate choice is independent of the moat. See [../architecture/tech-stack-analysis.md](../architecture/tech-stack-analysis.md) for the full evaluation.

We never sell "undo everything" - compensation is semantic and sometimes impossible. For irreversible actions we prefer a two-phase pattern (auth->capture->void) so that money never leaves before a confirmation. The flywheel is the moat-builder: an LLM proposes the inverse from the connector's OpenAPI, the round-trip harness verifies that A then A^-1 returns the system to its prior state, passing inverses are promoted to the version-pinned auto-runnable catalog, and each new customer inherits the accumulated library. The round-trip harness runs gVisor-isolated; air-gap-native round-trips replay recorded VCR-style cassettes offline, and a separate connected re-record station refreshes cassettes and runs oasdiff (OpenAPI diff) as a drift gate before an inverse is promoted to the auto-runnable catalog.

Considered: **Temporal as the substrate now** (a $5B durable-execution platform - kept as a pre-spiked contingency, not the MVP substrate: heavier operationally than needed at 0-3 months, and its compensation is a manual pattern rather than a library, so consuming it early buys mechanism we already get more cheaply from a Postgres-native substrate); **a hand-rolled saga pattern** (write our own orchestration - rejected: re-implementing durable, exactly-once, resumable execution is exactly the commodity mechanism a substrate gives away, and our IP is the content, not the plumbing); chose **DBOS Transact + Postgres for the mechanism plus a built compensation library** because it stands up the saga in a weekend and lets every engineering hour go to the version-pinned, FS-verified inverse catalog that is the actual moat.
...
- We carry a substrate dependency on DBOS + Postgres for the mechanism. Because it is held behind the gRPC ActionGuard seam plus a thin SagaCoordinator interface, with a pre-written Temporal adapter spike, a swap remains a contingency triggered only by multi-tenant fan-out / a Postgres ceiling / a buyer mandate (see [../tech-stack.md](../tech-stack.md), [../architecture/tech-stack-analysis.md](../architecture/tech-stack-analysis.md)) - not a scheduled migration, but still a non-trivial future re-platforming if a trigger fires.
- The gVisor-isolated, cassette-driven harness adds a connected re-record station and an oasdiff drift gate to operate, but this is what makes round-trip verification and promotion possible inside an air gap.
